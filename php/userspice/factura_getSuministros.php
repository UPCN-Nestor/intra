...
try{
	...
	$sql = "SELECT S.Soc_apelli, S.Soc_numero, SU.Sumi_numer, SU.Sumi_calle, SU.Sumi_nroca, SU.Sumi_piso, SU.Sumi_depto, ROUND(EXP(SUM(LOG(ISNULL(SV.TipoServ,1)))),0) Servicios, ISNULL(CS.Impago,0) Impago
		FROM UPCCOMPROD.dbo.SOCIOS S JOIN UPCCOMPROD.dbo.SUMINI SU ON (S.SucCodigo = SU.SucCodigo AND S.Soc_numero = SU.Soc_numero)
			LEFT JOIN (SELECT DISTINCT Sumi_numer, ISNULL(CASE Sum2Servi WHEN 1 THEN 2 WHEN 5 THEN 3 WHEN 10 THEN 5 ELSE 1 END, 1) AS TipoServ 
					FROM UPCCOMPROD.dbo.SUMSER WHERE Soc_numero = $soc AND Sum2Estad < 4) SV
				ON SV.Sumi_numer = SU.Sumi_numer
			LEFT JOIN (SELECT Sumi_numer, ROUND(SUM(CASE Comp_codig WHEN 'D' THEN CompSaldo ELSE -CompSaldo END), 2) Impago FROM UPCCOMPROD.dbo.COMSAL 
					WHERE Soc_numero = $soc AND Comp_tipo = 3 AND CompSaldo > 0 
					GROUP BY Sumi_numer) CS
				ON CS.Sumi_numer = SU.Sumi_numer
		WHERE S.Soc_numero = $soc
